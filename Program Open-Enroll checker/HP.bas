Sub colorcheckHp()
    ...
    Worksheets(3).Name = "Retention Status HP"
    Z = 2
    For i = 2 To trow1
        firtinst = False
        If Worksheets(1).Range("C" & i).Interior.Color = Worksheets(1).Range("K6").Interior.Color Then
            ...
            For j = 3 To trow2
                If (Replace(Worksheets(2).Range("B" & j).Value, " ", "") = asid) Then
                    ...
                    Worksheets(3).Range("D" & Z).Value = "retained opt out"
                    Z = Z + 1
                    GoTo cont
                End If
            Next
                If firstint = False Then
                    ...
                    Worksheets(3).Range("B" & Z).Value = Worksheets(1).Range("B" & i).Value
                    Worksheets(3).Range("C" & Z).Value = Worksheets(1).Range("C" & i).Value
                    Worksheets(3).Range("D" & Z).Value = "Did not retain opt out"
                    Z = Z + 1
            End If
        End If
cont:
    Next
     ...
     For i = 2 To trow3
        If Worksheets(3).Range("D" & i).Value = "retained opt out" Then
...
Sub reportH()
    ...
    For i = 2 To trow3
        If ActiveSheet.Range("D" & i).Value = "retained opt out" Then
            c1 = c1 + 1
        Else
            c2 = c2 + 1
        End If
